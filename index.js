...
class FlappyBird {
  ...
  gameState = false;
  pipe_sprite = document.querySelectorAll(".pipe_sprite");

  constructor() {
    if (!this.score_val && !this.message && !this.bird && !score_title) {
      alert("Elements not found.");
    }

    window.addEventListener("keydown", (e) => {
      // Start the game if enter key is pressed
      if (e.key == "Enter" && !this.gameState) {
        ...
        this.score_title.innerHTML = "Score : ";
        this.score_val.innerHTML = "0";
        this.gameLoop();
      }
    });

    document.addEventListener("keydown", (e) => {
      if (!this.currentlyJumping && (e.key == "ArrowUp" || e.key == " ")) {
        this.bird_dy = -3.4;
  ...
  gameLoop() {
    ...
    if (3 % 1 == 1) {
      ...
      this.pipe_sprite.forEach((element) => {
        let pipe_sprite_props = element.getBoundingClientRect();
        this.bird_props = bird.getBoundingClientRect();
        ...
        if (pipe_sprite_props.right <= 0) {
          ...
          if (
            this.bird_props.left <
              this.pipe_sprite_props.left + pipe_sprite_props.width &&
            this.bird_props.left + this.bird_props.width >
              pipe_sprite_props.left &&
            this.bird_props.top <
              pipe_sprite_props.top + pipe_sprite_props.height &&
            this.bird_props.top + this.bird_props.height > pipe_sprite_props.top
          ) {
            // Change game state and end the game
            // if collision occurs
            this.gameState = false;
            this.message.innerHTML = "Press Enter To Restart";
            ...
            if (
              pipe_sprite_props.right < this.bird_props.left &&
              pipe_sprite_props.right + this.background_Scrolling_Speed >=
                this.bird_props.left &&
              element.increase_score == "1"
            ) {
              this.score_val.innerHTML =
                Number.parseInt(this.score_val.innerHTML) + 1;
            }
            element.style.left =
              pipe_sprite_props.left - this.background_Scrolling_Speed + "px";
          }
        }
      });
    }
    this.bird_dy += this.gravity;
    if (this.bird_props.top <= 0 || this.bird_props.bottom >= innerHeight) {
      this.gameState = false;
      this.message.innerHTML = "Press Enter To Restart";
      this.message.style.left = "28vw";
      return;
    }
    this.bird.style.top = this.bird_props.top + this.bird_dy + "px";
    this.bird_props = this.bird.getBoundingClientRect();
...
function play() {
  ...
  function apply_gravity() {
    ...
    document.addEventListener("keydown", (e) => {
      if (!currentlyJumping && (e.key == "ArrowUp" || e.key == " ")) {
        bird_dy = -0.4;
    ...
    if (bird_props.top <= 0 || bird_props.bottom >= innerHeight) {
      game_state = "End";
      message.innerHTML = "Press Enter To Restart";
      message.style.left = "28vw";
      return;
